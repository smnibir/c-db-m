...
function render_clickup_client_dashboard() {
    ...
    // Determine display name and tooltip for long usernames
    $display_name = $user_name;
    $tooltip_title = '';
    $max_length = 20;

    if (mb_strlen($user_name) > $max_length) {
        // If full name is too long, try first name only
        if (mb_strlen($first_name) <= $max_length) {
            $display_name = $first_name;
            $tooltip_title = $user_name;
        } else {
            // If even first name is too long, truncate it
            $display_name = mb_substr($first_name, 0, $max_length - 3) . '...';
            $tooltip_title = $user_name;
        }
    }
    ...
    $company_tooltip = '';
    if (mb_strlen($company_name) > $max_length) {
        $display_company = mb_substr($company_name, 0, $max_length - 3) . '...';
        $company_tooltip = $company_name;
    }
    ...
    <style>
        ...
        /* Tooltip styling for long usernames and company names */
        .user-dp .text-icon[title] {
            cursor: help;
            position: relative;
        }
        .user-dp .text-icon[title]:hover {
        ...
        <div class="clickup-sidebar">
            <div class="wg-branding flex" style="justify-content: space-between;">
                <div class="flex" style="gap:4px">
                    <div>
                        <img src="<?php echo esc_url('https://portal.webgrowth.io/wp-content/uploads/2025/07/webgrowth.png'); ?>" alt="Webgrowth">
                    </div>
            ...
            <div class="flex user-profile-section" style="position: absolute; bottom: 0rem; width: calc(100% - 3rem);">
                <div class="flex user-profile" style="align-items: center; padding: .75rem 1rem; cursor: pointer; width: 100%;">
                    ...
                    <div class="flex user-dp" style="flex-direction: column; flex-grow: 1;">
                        <span class="text-icon" style="font-size: 1rem;margin: -5px 0" <?php if ($tooltip_title) echo 'title="' . esc_attr($tooltip_title) . '"'; ?>><?php echo esc_html($display_name); ?></span>
                        <span class="text-icon sec-color" style="font-size: .8rem; line-height: .75rem;margin-top: 5px;" <?php if ($company_tooltip) echo 'title="' . esc_attr($company_tooltip) . '"'; ?>><?php echo esc_html($display_company); ?></span>
                    </div>
                    ...
                    item.addEventListener('click', function (e) {
                        ...
                        switch (action) {
                            case 'logout':
                                ...
                                break;
                            case 'edit-profile':
                                window.location.href = '/account/edit-account';
                                break;
                            case 'portal':
