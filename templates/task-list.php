            ...
            <div style="margin-left: 2.5rem;">
                ...
                <div class="task-meta">
                    ...
                    <span class="task-date">
                        ...
                        <span>Created <?= date('Y-m-d', intval($task['date_created'] / 1000)) ?></span>
                    </span>
                    <?php if (!empty($task['due_date'])): ?>
                        <span class="task-date task-due-date">
                            <svg xmlns="http://www.w3.org/2000/svg" width="1.2rem" height="1.2rem" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-calendar-clock w-4 h-4 text-muted-foreground"><path d="M21 7.5V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h3.5"></path><path d="M16 2v4"></path><path d="M8 2v4"></path><path d="M3 10h5"></path><path d="M17.5 17.5 16 16.25V14"></path><circle cx="16" cy="16" r="6"></circle></svg>
                            <span>Due <?= date('Y-m-d', intval($task['due_date'] / 1000)) ?></span>
                        </span>
                    <?php endif; ?>
                    <?php 
                    // Check for recurring task
                    $is_recurring = !empty($task['recurrence']);
                    if ($is_recurring && !empty($task['recurrence']['next_occurrence'])):
                    ?>
                        <span class="task-date task-recurring-date">
                            <svg xmlns="http://www.w3.org/2000/svg" width="1.2rem" height="1.2rem" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-refresh-cw w-4 h-4 text-muted-foreground"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"></path><path d="M21 3v5h-5"></path><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"></path><path d="M8 16H3v5"></path></svg>
                            <span>Next <?= date('Y-m-d', intval($task['recurrence']['next_occurrence'] / 1000)) ?></span>
                        </span>
                    <?php endif; ?>
                </div>
            </div>
